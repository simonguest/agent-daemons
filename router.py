...
class MessageRouter:
    def __init__(self, registry, global_queue, agent_queues: Dict[uuid.UUID, Queue]):
        logger.remove()
        logger.add(
            sys.stdout,
            colorize=True,
    ...
    async def _route_message(self, message):
        self.router_logger.info(
            f"Routing message from {message.get('from', 'unknown')} to {message.get('to', 'unknown')}"
        )
        target = message.get("to")

        if target:
            try:
                # Convert target to UUID
                target_uuid = (
                ...
                if target_uuid in self.agent_queues:
                    ...
                    self.router_logger.info(f"Sent message to {target_uuid}")
                else:
                    self.router_logger.error(
                        f"Agent {target_uuid} not found in agent_queues"
                    )
